...
#define LCD_CMD ((rt_uint32_t)0x6001FFFF)
#define LCD_DATA ((rt_uint32_t)0x60020000)

#define LCD_BL 30    // PB14
#define LCD_RESET 61 // PD13
...
{
    *(__IO rt_uint8_t *)LCD_CMD = cmd;
    return RT_EOK;
}

static rt_err_t lcd_write_data(const rt_uint8_t data)
{
    *(__IO rt_uint8_t *)LCD_DATA = data;
    return RT_EOK;
}

static rt_err_t lcd_write_half_word(const rt_uint16_t da)
{
    *(__IO rt_uint8_t *)LCD_DATA = (rt_uint8_t)(da >> 8);
    *(__IO rt_uint8_t *)LCD_DATA = (rt_uint8_t)da;
    return RT_EOK;
}

static rt_err_t lcd_fsmc_init(void)
{
...
static void lcd_gpio_init(void)
{
    ...
    rt_pin_write(LCD_BL, PIN_HIGH); // back light

    rt_pin_mode(LCD_RESET, PIN_MODE_OUTPUT);
    rt_pin_write(LCD_RESET, PIN_LOW);
...
void lcd_clear(rt_uint16_t color)
{
    ...
    if (buf)
    {
        ...
        for (i = 0; i < 20; i++)
        {
            for (j = 0; j < LCD_CLEAR_SEND_NUMBER; j++)
            {
                *(__IO rt_uint8_t *)LCD_DATA = buf[j];
            }
        }
        rt_free(buf);
    }
    else
    {
        ...
        for (i = 0; i < LCD_W; i++)
        {
            for (j = 0; j < LCD_H; j++)
            {
                *(__IO rt_uint8_t *)LCD_DATA = data[0];
                *(__IO rt_uint8_t *)LCD_DATA = data[1];
            }
        }
    }
}
...
void lcd_fill(rt_uint16_t x_start, rt_uint16_t y_start, rt_uint16_t x_end, rt_uint16_t y_end, rt_uint16_t color)
{
    ...
    if (fill_buf)
    {
        ...
        while (1)
        {
            ...
            for (i = 0; i < size; i++)
            {
                *(__IO rt_uint8_t *)LCD_DATA = fill_buf[i];
            }
...
void lcd_draw_line(rt_uint16_t x1, rt_uint16_t y1, rt_uint16_t x2, rt_uint16_t y2)
{
    ...
    if (y1 == y2)
    {
        ...
        for (i = 0; i < (x2 - x1) * 2; i++)
        {
            *(__IO rt_uint8_t *)LCD_DATA = line_buf[i];
        }
...
static void lcd_show_char(rt_uint16_t x, rt_uint16_t y, rt_uint8_t data, rt_uint32_t size)
{
    ...
    if (size == 16)
    {
        ...
        else
        {
            ...
            {
                *(__IO rt_uint8_t *)LCD_DATA = font_buf[i];
            }
            rt_free(font_buf);
        }
    }
