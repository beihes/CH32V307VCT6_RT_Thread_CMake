...
#define SDIO_BUS_CONFIG                       \
    {                                         \
        .Instance = SDIO,                     \
        .dma_rx.dma_rcc = RCC_AHBPeriph_DMA2, \
        .dma_tx.dma_rcc = RCC_AHBPeriph_DMA2, \
...
static struct ch32_sdio_config sdio_config = SDIO_BUS_CONFIG;
static struct ch32_sdio_class sdio_obj;
...
static rt_uint32_t ch32_sdio_clk_get(struct ch32_sdio *hw_sdio)
{
...
static void rthw_sdio_wait_completed(struct rthw_sdio *sdio)
{
    ...
    struct ch32_sdio *hw_sdio = sdio->sdio_des.hw_sdio;
 ...
 * @param  sdio_des  ch32_sdio_des
 * @retval rt_mmcsd_host
 */
struct rt_mmcsd_host *sdio_host_create(struct ch32_sdio_des *sdio_des)
{
    struct rt_mmcsd_host *host;
    ...
    rt_memcpy(&sdio->sdio_des, sdio_des, sizeof(struct ch32_sdio_des));
    sdio->sdio_des.hw_sdio = (sdio_des->hw_sdio == RT_NULL ? (struct ch32_sdio *)SDIO_BASE_ADDRESS : sdio_des->hw_sdio);
    sdio->sdio_des.clk_get = (sdio_des->clk_get == RT_NULL ? ch32_sdio_clk_get : sdio_des->clk_get);
...
void SD_LowLevel_DMA_TxConfig(uint32_t *src, uint32_t *dst, uint32_t BufferSize)
{
    ...
    sdio_obj.dma.handle_tx.Init.DMA_MemoryBaseAddr = (uint32_t)src;
    sdio_obj.dma.handle_tx.Init.DMA_PeripheralBaseAddr = (uint32_t)dst;
    sdio_obj.dma.handle_tx.Init.DMA_DIR = DMA_DIR_PeripheralDST;
    sdio_obj.dma.handle_tx.Init.DMA_BufferSize = BufferSize;
    sdio_obj.dma.handle_tx.Init.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
    sdio_obj.dma.handle_tx.Init.DMA_MemoryInc = DMA_MemoryInc_Enable;
    sdio_obj.dma.handle_tx.Init.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;
    sdio_obj.dma.handle_tx.Init.DMA_MemoryDataSize = DMA_MemoryDataSize_Word;
    sdio_obj.dma.handle_tx.Init.DMA_Mode = DMA_Mode_Normal;
    sdio_obj.dma.handle_tx.Init.DMA_Priority = DMA_Priority_Medium;
    sdio_obj.dma.handle_tx.Init.DMA_M2M = DMA_M2M_Disable;

    DMA_Init(sdio_obj.dma.handle_tx.Instance, &sdio_obj.dma.handle_tx.Init);
    DMA_Cmd(sdio_obj.dma.handle_tx.Instance, ENABLE);
}
...
void SD_LowLevel_DMA_RxConfig(uint32_t *src, uint32_t *dst, uint32_t BufferSize)
{
    ...
    sdio_obj.dma.handle_rx.Init.DMA_MemoryBaseAddr = (uint32_t)dst;
    sdio_obj.dma.handle_rx.Init.DMA_PeripheralBaseAddr = (uint32_t)src;
    sdio_obj.dma.handle_rx.Init.DMA_DIR = DMA_DIR_PeripheralSRC;
    sdio_obj.dma.handle_rx.Init.DMA_BufferSize = BufferSize;
    sdio_obj.dma.handle_rx.Init.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
    sdio_obj.dma.handle_rx.Init.DMA_MemoryInc = DMA_MemoryInc_Enable;
    sdio_obj.dma.handle_rx.Init.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;
    sdio_obj.dma.handle_rx.Init.DMA_MemoryDataSize = DMA_MemoryDataSize_Word;
    sdio_obj.dma.handle_rx.Init.DMA_Mode = DMA_Mode_Normal;
    sdio_obj.dma.handle_rx.Init.DMA_Priority = DMA_Priority_Medium;
    sdio_obj.dma.handle_rx.Init.DMA_M2M = DMA_M2M_Disable;

    DMA_Init(sdio_obj.dma.handle_rx.Instance, &sdio_obj.dma.handle_rx.Init);
    DMA_Cmd(sdio_obj.dma.handle_rx.Instance, ENABLE);
}

/**
 * @brief  This function get ch32 sdio clock.
 * @param  hw_sdio: ch32_sdio
 * @retval PCLK2Freq
 */
static rt_uint32_t ch32_sdio_clock_get(struct ch32_sdio *hw_sdio)
{
    RCC_ClocksTypeDef rcc_clk = {0};
...
int rt_hw_sdio_init(void)
{
    ...
    NVIC_SetPriority(SDIO_IRQn, 0x2 << 5);
    NVIC_EnableIRQ(SDIO_IRQn);

    sdio_des.clk_get = ch32_sdio_clock_get;
    sdio_des.hw_sdio = (struct ch32_sdio *)SDCARD_INSTANCE;
