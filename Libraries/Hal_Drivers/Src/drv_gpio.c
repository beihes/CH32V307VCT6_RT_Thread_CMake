...
static rt_err_t ch32_pin_attach_irq(struct rt_device *device, rt_int32_t pin, rt_uint32_t mode, void (*hdr)(void *args), void *args)
{
    const struct pin_index *index;
    rt_base_t level;
...
static rt_err_t ch32_pin_irq_enable(struct rt_device *device, rt_base_t pin, rt_uint32_t enabled)
{
    const struct pin_index *index;
    ...
    if (enabled == PIN_IRQ_ENABLE)
    {
        ...
        if (index->gpio == GPIOA)
            gpio_port_souce = GPIO_PortSourceGPIOA;
        if (index->gpio == GPIOB)
            gpio_port_souce = GPIO_PortSourceGPIOB;
        if (index->gpio == GPIOC)
            gpio_port_souce = GPIO_PortSourceGPIOC;
        if (index->gpio == GPIOD)
            gpio_port_souce = GPIO_PortSourceGPIOD;
        if (index->gpio == GPIOE)
            gpio_port_souce = GPIO_PortSourceGPIOE;
